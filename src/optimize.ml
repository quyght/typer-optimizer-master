...
and strreduc (e : EL.elexp)
	  : EL.elexp
    = match e with
		| Call(fct, args) ->
       (match (strreduc fct, List.map strreduc args) with
         | (Var((l,"_*_"),i), [Imm(Integer(_,n));Imm(Integer(_,2))])
	 		-> Call(Var((l,"_+_"),i), [Imm(Integer(l,n));Imm(Integer(l,n))])
 		 | (Var((l,"_*_"),i), [Imm(Integer(_,2));Imm(Integer(_,n))])
	 		-> Call(Var((l,"_+_"),i), [Imm(Integer(l,n));Imm(Integer(l,n))])
		 | _ -> e		
		)
		| _ -> e
